...
type ElementAttributes<P extends HTMLElement> =
    | Partial<Omit<P, keyof Node>>
    | {
          style: Partial<CSSStyleDeclaration> | undefined;
          onrender?: (elem: P) => void;
      };

type ElementFactory = {
    <K extends keyof HTMLElementTagNameMap>(name: K): (
        options?: ElementAttributes<HTMLElementTagNameMap[K]>
    ) => (...args: HTMLChild[]) => HTMLElementTagNameMap[K];

    (name: string): (
        options?: ElementAttributes<HTMLElement>
    ) => (...args: HTMLChild[]) => HTMLElement;
};

const _elementFactory: ElementFactory = (name: string | keyof HTMLElementTagNameMap) => {
    return (options: ElementAttributes<HTMLElement> = {}) => {
...
const body = (...elements: HTMLChild[]) => {
    document.body.append(...elements);
};

const define = <T extends { [key: string]: any }>(
    defaultProps: T,
    renderFunction: (props: Partial<T>, children: HTMLChild[]) => HTMLChild
) => {
    return (options = {}) => {
        return (...args: HTMLChild[]) => {
...
const dedent = (text: TemplateStringsArray) => {
    ...
    let smallestIndent: number | undefined = undefined;
