...
class AsciiDocument {
    constructor(target, beforeRender) {
        this.target = target;
...
const centerBlock = (text, width) => {
    return text
        .split('\n')
        .map((p) => centerText(p, width))
        .join('\n');
};
const writeBlock = (frame, x, y, content, options) => {
    let lines;
...
const writeFrame = (frame, x, y, contentFrame) => {
